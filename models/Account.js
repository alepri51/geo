...
class Role extends Node {
    ...
    static async byName({ name, service_name }) {
        let roles = await this.initialize({ service_name });
        let role = roles.find(role => role.name === name);

        if(!role)
            throw { code: 404, message: `Role '${name}' not found.`}
...
class Shadow extends Account {
    ...
    static async findOrCreate({ _id, name, email, avatar, service_name }) {
        ...
        if(!shadow) {
            let role = await Role.byName({ name: 'Anonymous', service_name });
